...
def radar_chart(scores: dict, title: str, figsize=(7, 7)):
    """scores: dict dim->0-100. Mengembalikan PNG bytes."""
    labels = [DIMS[d]["radar"] for d in DIM_ORDER]
    vals = [scores[d] for d in DIM_ORDER]
    N = len(labels)
...
def bar_chart_targets(mean_scores: dict, title: str, figsize=(9, 6)):
...
def dept_chart(df_scores, dept_map: dict, title: str, figsize=(9, 6)):
    ...
    if not depts:
        return None
    means = df.groupby("dept")[DIM_ORDER].mean().reindex(depts)
    x = np.arange(len(DIM_ORDER))
    fig, ax = plt.subplots(figsize=figsize)
    width = 0.8 / len(depts)
    palette = plt.cm.tab10.colors
    for i, dept in enumerate(depts):
        ax.bar(x + i*width, means.loc[dept, DIM_ORDER], width=width, label=dept, color=palette[i % 10])
    ax.set_xticks(x + width*(len(depts)-1)/2)
    ax.set_xticklabels([DIMS[d]["radar"] for d in DIM_ORDER], fontsize=8, rotation=30, ha="right")
